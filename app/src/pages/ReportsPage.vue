...
<template>
  <div class="p-4 md:p-6 lg:p-8 space-y-5">
    ...
    <!-- Tabs -->
    <div class="flex gap-1 p-0.5 rounded-lg w-fit" :style="{ background: 'var(--bg-input)' }">
    ...
    <template v-if="activeTab === 'performance'">
      ...
      <div v-else-if="performanceData.length === 0" class="rounded-xl p-8 text-center" :style="{ background: 'var(--bg-card)', border: '1px solid var(--border)' }">
        <v-icon icon="mdi-chart-bar" size="40" :style="{ color: 'var(--text-faint)' }" />
        <p class="text-sm mt-3" :style="{ color: 'var(--text-muted)' }">{{ t('common.noData') }}</p>
        <p class="text-xs mt-1" :style="{ color: 'var(--text-faint)' }">{{ teamStore.teams.length === 0 ? t('team.noTeamsYet') : t('dashboard.createTasksHint') }}</p>
      </div>

      <template v-else>
        <!-- Table -->
        <div class="rounded-xl overflow-hidden" :style="{ background: 'var(--bg-card)', border: '1px solid var(--border)' }">
          <div class="grid grid-cols-[auto_1fr_auto_auto_auto_auto_1fr] gap-x-4 text-[10px] font-semibold uppercase tracking-widest px-4 py-2.5" :style="{ color: 'var(--text-muted)', borderBottom: '1px solid var(--border)' }">
          ...
          <div v-for="(m, i) in performanceData" :key="m.userId" class="grid grid-cols-[auto_1fr_auto_auto_auto_auto_1fr] gap-x-4 items-center px-4 py-3" :style="{ borderBottom: '1px solid var(--border)' }">
            <span class="text-xs font-medium w-4" :style="{ color: 'var(--text-muted)' }">{{ i + 1 }}</span>
            <div class="flex items-center gap-2.5">
              ...
              <span class="text-sm font-medium truncate" :style="{ color: 'var(--text)' }">{{ m.userName }}</span>
            </div>
            ...
            <span class="text-xs w-8 text-center" :style="{ color: 'var(--text-secondary)' }">{{ m.completed }}</span>
        ...
        <div class="rounded-2xl p-5" :style="{ background: 'var(--bg-card)', border: '1px solid var(--border)' }">
          ...
          <div class="flex justify-center gap-4 mt-4">
            <div class="flex items-center gap-1.5"><div class="w-2.5 h-2.5 rounded-full bg-success" /><span class="text-[11px]" :style="{ color: 'var(--text-secondary)' }">{{ t('reports.onTimeCol') }}</span></div>
            <div class="flex items-center gap-1.5"><div class="w-2.5 h-2.5 rounded-full bg-error" /><span class="text-[11px]" :style="{ color: 'var(--text-secondary)' }">{{ t('reports.late') }}</span></div>
          </div>
        </div>
      </template>
    </template>
    ...
    <!-- Incoming / Sent -->
    <div v-if="activeTab === 'incoming' || activeTab === 'sent'" class="rounded-xl overflow-hidden" :style="{ background: 'var(--bg-card)', border: '1px solid var(--border)' }">
      <div v-for="report in (activeTab === 'incoming' ? incomingReports : sentReports)" :key="report.id" class="flex items-center gap-3 px-4 py-3" :style="{ borderBottom: '1px solid var(--border)' }">
        ...
        <div class="flex-1 min-w-0">
          <p class="text-sm truncate" :style="{ color: 'var(--text)' }">{{ activeTab === 'incoming' ? t('reports.requestedReport', { name: report.requester?.name ?? t('reports.unknown') }) : t('reports.reportFrom', { name: report.targetUser?.name ?? t('reports.unknown') }) }}</p>
          <p class="text-[11px]" :style="{ color: 'var(--text-muted)' }">{{ report.message || t('reports.noMessage') }} · {{ formatRelativeTime(report.createdAt) }}</p>
        </div>
      ...
      <div v-if="(activeTab === 'incoming' ? incomingReports : sentReports).length === 0" class="py-8 text-center text-xs" :style="{ color: 'var(--text-muted)' }">{{ t('reports.noReports') }}</div>
    </div>
    ...
    <v-dialog v-model="showRespondModal" max-width="420">
      <v-card class="pa-5" color="surface">
        ...
        <p class="text-xs mb-3" :style="{ color: 'var(--text-muted)' }">{{ respondingReport?.message }}</p>
