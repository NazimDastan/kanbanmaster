...
const emit = defineEmits<{ markRead: [id: string]; markAllRead: []; navigate: [referenceId: string, type: string] }>()
...
<template>
  <v-menu :close-on-content-click="false" max-width="360">
    <template #activator="{ props }">
      <button v-bind="props" class="relative p-2 rounded-lg hover:bg-white/5 transition-colors" aria-label="Notifications">
    ...
    <v-card min-width="340" max-height="440" color="surface" class="rounded-xl border border-white/5">
      ...
      <div v-if="notifications.length > 0" class="overflow-y-auto max-h-[340px]">
        <div v-for="n in notifications.slice(0, 20)" :key="n.id" class="flex items-start gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors cursor-pointer" :class="{ 'bg-primary/5': !n.isRead }" @click="emit('markRead', n.id); if (n.referenceId) emit('navigate', n.referenceId, n.type)">
