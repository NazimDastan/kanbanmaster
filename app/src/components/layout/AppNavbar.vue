...
<template>
  <header class="h-14 flex-shrink-0 flex items-center justify-between px-4 md:px-6 border-b" style="background: var(--bg-card, #0f0f1a); border-color: var(--border, rgba(255,255,255,0.05))">
    <div class="flex items-center gap-3">
      ...
      <span class="text-sm font-bold tracking-tight hidden sm:block">KanbanMaster</span>
    </div>

    <div class="flex items-center gap-1">
      <!-- Theme toggle -->
      <button class="p-2 rounded-lg hover:bg-white/5 transition-colors" @click="toggleTheme">
        <v-icon :icon="theme === 'dark' ? 'mdi-weather-sunny' : 'mdi-weather-night'" size="18" class="text-white/40" />
      </button>
      ...
      <v-menu>
        <template #activator="{ props }">
          <button v-bind="props" class="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-white/5 transition-colors">
            ...
            <span class="text-xs font-medium text-white/80 hidden md:block">{{ authStore.userName }}</span>
            <v-icon icon="mdi-chevron-down" size="14" class="text-white/40" />
          </button>
        </template>
        <v-list density="compact" min-width="160" bg-color="#1e1e32" class="rounded-xl border border-white/5">
